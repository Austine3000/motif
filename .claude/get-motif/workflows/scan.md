...
- If `{"error": "missing"}` or `{"error": "corrupt"}`: run `node .claude/get-motif/scripts/motif-state.js recover`. If recovery succeeds, notify user: "State recovered from artifacts -- phase: {phase}, {N}/{M} screens". If recovery fails: this is fine for scan -- it is a utility command that works without state.
- Otherwise: state is loaded.

No phase validation needed -- scan is a utility command that runs in any phase.
No prerequisite artifacts needed -- scan discovers them.
</gate_check>
...
Run `node .claude/get-motif/scripts/motif-state.js update last_command /motif:scan` and `update last_outcome success`.
