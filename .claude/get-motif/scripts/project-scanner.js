...
function detectFramework(projectRoot) {
  ...
  if (!pkg) {
    return {
      ...
      adoption: {
        action: 'greenfield',
        platform: null,
        reason: 'No package.json detected, so framework signatures are unavailable.',
      },
      signatures: {
        next: false,
        vite: false,
  ...
  const hasDependency = (name) => Boolean(allDeps[name]);
  const hasAnyDependency = (names) => names.some((name) => hasDependency(name));
  const hasFile = (filename) => fs.existsSync(path.join(projectRoot, filename));
  const firstDependencyVersion = (names) => {
    for (const name of names) {
      if (allDeps[name]) return allDeps[name];
    }
    return null;
  };

  const nextConfigFiles = ['next.config.js', 'next.config.mjs', 'next.config.ts', 'next.config.cjs'];
  const viteConfigFiles = ['vite.config.js', 'vite.config.mjs', 'vite.config.ts', 'vite.config.cjs'];
  const expoConfigFiles = ['app.json', 'app.config.js', 'app.config.ts'];

  const hasNextDependency = hasDependency('next');
  const hasViteDependency = hasDependency('vite') || hasDependency('@vitejs/plugin-react');
  const hasExpoDependency = hasDependency('expo');
  const hasReactNativeDependency = hasAnyDependency(['react-native', '@expo/vector-icons']);
  const hasReactDependency = hasDependency('react');

  const hasNextConfig = nextConfigFiles.some((file) => hasFile(file));
  const hasViteConfig = viteConfigFiles.some((file) => hasFile(file));
  const hasExpoConfigFile = expoConfigFiles.some((file) => hasFile(file));

  let appJsonHasExpo = false;
  const appJson = safeReadJSON(path.join(projectRoot, 'app.json'));
  if (appJson && typeof appJson === 'object' && appJson.expo && typeof appJson.expo === 'object') {
    appJsonHasExpo = true;
  }

  const signatures = {
    next: hasNextDependency || hasNextConfig,
    vite: hasViteDependency || hasViteConfig,
    expo: (hasExpoDependency || hasReactNativeDependency) && (hasExpoConfigFile || appJsonHasExpo),
    react: hasReactDependency,
    reactNative: hasReactNativeDependency,
  };

  const supportedMatches = [
    signatures.next ? 'web-nextjs' : null,
    signatures.vite ? 'web-vite' : null,
    signatures.expo ? 'mobile-expo' : null,
  ].filter(Boolean);

  if (supportedMatches.length === 1) {
    const platform = supportedMatches[0];
    let name = platform;
    let version = null;
    let reason = '';
    if (platform === 'web-nextjs') {
      name = 'next';
      version = firstDependencyVersion(['next']);
      reason = 'Detected Next.js dependency/config signatures.';
    } else if (platform === 'web-vite') {
      name = 'vite';
      version = firstDependencyVersion(['vite']);
      reason = 'Detected Vite dependency/config signatures.';
    } else if (platform === 'mobile-expo') {
      name = 'expo';
      version = firstDependencyVersion(['expo']);
      reason = 'Detected Expo/React Native signatures with Expo app config.';
    }
  ...
  if (supportedMatches.length > 1) {
    return {
      name: 'react',
      version: firstDependencyVersion(['react']),
      confidence: 'MEDIUM',
      language,
      classification: 'ambiguous-react',
      adoption: {
        action: 'confirm',
        platform: null,
        reason: `Conflicting framework signatures detected (${supportedMatches.join(', ')}).`,
      },
      signatures,
    };
  }

  if (signatures.react || signatures.reactNative) {
    return {
      name: 'react',
      version: firstDependencyVersion(['react']),
      confidence: 'MEDIUM',
      ...
      adoption: {
        action: 'confirm',
        platform: null,
        reason: 'React detected without a clear supported framework signature.',
      },
      signatures,
    };
  }
  ...
  return {
    ...
    adoption: {
      action: 'greenfield',
      platform: null,
      reason: 'No supported framework signatures were detected.',
    },
    signatures,
  };
}
...
function writeProjectScan(outputDir, scanData) {
  ...
  lines.push(`- **Classification:** ${framework.classification || 'unknown'}`);
  lines.push('');

  lines.push('## Brownfield Adoption Signal');
  lines.push(`- **Action:** ${framework.adoption && framework.adoption.action ? framework.adoption.action : 'greenfield'}`);
  lines.push(`- **Platform:** ${framework.adoption && framework.adoption.platform ? framework.adoption.platform : 'none'}`);
  lines.push(`- **Reason:** ${framework.adoption && framework.adoption.reason ? framework.adoption.reason : 'No adoption signal available.'}`);
  lines.push('- **Signature Map:**');
  lines.push(`  - next: ${framework.signatures && framework.signatures.next ? 'yes' : 'no'}`);
  lines.push(`  - vite: ${framework.signatures && framework.signatures.vite ? 'yes' : 'no'}`);
  lines.push(`  - expo: ${framework.signatures && framework.signatures.expo ? 'yes' : 'no'}`);
  lines.push(`  - react: ${framework.signatures && framework.signatures.react ? 'yes' : 'no'}`);
  lines.push(`  - react-native: ${framework.signatures && framework.signatures.reactNative ? 'yes' : 'no'}`);
  lines.push('');
...
function scan(projectRoot, outputDir) {
  ...
  console.log(`Adoption: ${framework.adoption.action}${framework.adoption.platform ? ` -> ${framework.adoption.platform}` : ''}`);
